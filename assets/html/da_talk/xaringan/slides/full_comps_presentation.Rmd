...
### **Max Chumley** <br> &mdash; <br> Mechanical Engineering<br><br>Computational Mathematics<br>Science and Engineering<br> &mdash;<br>Date: 3-19-24
...
???
To study the idea of differentiability in the space of persistence diagrams it is useful to think of persistence as a map. If we start with a point cloud theta, we map this point cloud to the persistence diagram using the relevant filter function. 

In this case we have one loop that is born at r=b and dies at r=sqrt(2)b using a vietoris rips filtration.

The persistence diagram is then mapped to a real valued feature with the map V. Here I am showing the total persistence feature which quantifies how far points are from the diagonal. 
...
I will point out that each step requires M samples to approximate noise statistics. M was taken to be 1000 in the paper at the bottom. 
...
The new forecast makes up the analysis which is fed back into the loop to be used as the model in the next assimilation window and the process is continued as new observations stream in. 
...
???
For my final topic of this chapter I will show how I used persistent homology to perform dynamic state detection on network representations of time series signals. 
...
![:img 95%, 2.5%, 30%](../Figs/goals/goals1.png)
...
--
![:img 95%, 2.5%, 30%](../Figs/goals/goals2.png)
--
![:img 95%, 2.5%, 30%](../Figs/goals/goals3.png)
--
![:img 95%, 2.5%, 30%](../Figs/goals/goals4.png)
